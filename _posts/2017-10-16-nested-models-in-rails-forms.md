...
First, a bit about the database schema. Here, I will talk about two models, `routines` and `exercises`. An exercise routine has a name and a description, it belongs to a user, and it consists (has_many) of a number of exercises. Each exercise has a name, a number of sets, a number of reps, and a load, as well the routine that it belongs to (as a foreign key). 
...
This form was created using simple_form gem.The simple_form documentation has a good description of nested models here (https://github.com/plataformatec/simple_form/wiki/Nested-Models). I followed this documentation to come up with the code below. 
